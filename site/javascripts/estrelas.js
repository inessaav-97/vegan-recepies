function getKey() { return 'rating_' + window.location.pathname; }
...
function renderStars(container, saved) {
  ...
  if (!saved) {
    stars.forEach(s => {
      s.addEventListener('mouseover', () => { const v = +s.dataset.value; stars.forEach(x => x.classList.toggle('hover', +x.dataset.value <= v)); });
      s.addEventListener('mouseout', () => stars.forEach(x => x.classList.remove('hover')));
      s.addEventListener('click', () => { localStorage.setItem(getKey(), s.dataset.value); renderStars(container, +s.dataset.value); });
    });
  }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const img = document.querySelector('.md-typeset img');
  const anchor = img ? (img.closest('p') || img) : document.querySelector('.md-typeset h1');
